...
v1.0からはmasterに何かをマージするたびにバージョンタグが上がるようになっています。
サブモジュールでレポジトリに入れるのは避けたいと考えているので、ローカルのsrt.elより
masterのバージョンが上の場合、上書きしてください。
...
~srt-deftest~ はテスト名とテスト設定を受け取り、内部の ~srt-test-cases~ に ~add-to-list~ します。
